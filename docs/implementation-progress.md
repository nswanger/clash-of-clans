...
| 10. End-to-end acceptance and production handoff | In progress | Fixture acceptance, Pages publication, Discord OAuth, Nick's admin bootstrap, UnRaid replacement, local verification, and operations guidance are complete. The idle-CWL dashboard fix is implemented locally; recommendation persistence, automatic auditing, cleanup scheduling, and active-CWL acceptance remain. |
...
- `pnpm test`: 135 Vitest tests passed across the collector, domain, recommendations, and web packages; the verification-script shell suite also passed.
...
- Authenticated production smoke before the idle-dashboard fix is deployed: `Line Em Up` resolves correctly, the current root route reproduces unavailable freshness and empty CWL placeholders, the `Access` route loads Nick as `admin`, and the browser reports no console warnings or errors.
...
Task 10 is in progress on `codex/cwl-assistant-mvp`. GitHub Pages serves the production build at `https://nswanger.github.io/clash-of-clans/`. UnRaid runs immutable Task 10 image `cwl-collector:65e623102350` with protected configuration under `/mnt/user/appdata/cwl-collector` and no published ports.
...
- The local chain passes: Supabase reset and 55 pgTAP checks, workspace typecheck, 135 tests plus collector verification scripts, production build, 16 Playwright scenarios, and the `cwl-collector:acceptance` image build.
...
- The production collector has raw clan and roster data, but canonical CWL tables are empty while CWL is inactive. The local dashboard fix now carries raw clan members and current collection health through the no-season state and replaces war placeholders with an explicit clan overview.
- No real recommendation was approved or overridden.

Five Alpha-to-MVP gates remain:

1. Deploy and verify the idle-CWL overview in Nick's authenticated production session, including the `Access` admin route.
2. Invoke `OrderedRulesStrategy` from a production derivation job and persist versioned recommendation rows.
3. Create append-only audit events automatically for invitations, role changes, availability changes, and recommendation decisions.
4. Configure and verify the daily Supabase Cron call to `purge_expired_raw_snapshots()`.
5. During the next active CWL, verify nonzero canonical normalization, availability entry, recommendation generation, human approval or override, audit visibility, and retry idempotency. Obtain Nick's explicit choice before approving or overriding a real recommendation.
