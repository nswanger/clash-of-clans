...
- [ ] Run `supabase migration list` and confirm every local migration has a matching remote entry. Pages deploys on merge and the database does not, so an unapplied migration surfaces as a runtime failure on whichever route reads it.
