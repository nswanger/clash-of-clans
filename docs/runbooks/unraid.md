...
2. From the same local shell where `commit_sha` was set, import the image and prepare protected configuration:
   ...
   - `IDLE_INTERVAL_HOURS`; positive integer, default `24`. During the first ten
     days of a month the collector ignores this and polls at the active cadence
     until it has seen that month's CWL season, so sign-up day is collected within
     an hour rather than after the next idle tick.
