...
**The post-CWL review phase is locked** ([#54](https://github.com/nswanger/clash-of-clans/issues/54)), in [`prototype/cwl-review.html`](prototype/cwl-review.html) ([published](https://claude.ai/code/artifact/6809afd0-916a-44c5-805f-5c3aedbd0216)). It is the second phase of the CWL route rather than a fourth tab ([ADR 0002](../docs/decisions/0002-app-surfaces-and-cwl-phase.md)), and the first surface here that had to be designed rather than ported — the other two already existed in `apps/web`.

- **Three decisions, one ranked list.** Bonus medals, role changes and follow-ups share a roster and a season, and [ADR 0001](../docs/decisions/0001-cwl-evidence-and-bonus-priority.md)'s ranking serves all three from opposite ends: the top of the list is who contributed most, the foot is who did not turn up. A separate follow-up section would put the same rows on the page twice, which is what `#/overview` is being deleted for — and the CWL roster is 24 members, not the roster's 50, so the foot of the list is a flick away rather than buried.
- **Two groups, and they are not a bonus cutoff.** The boundary is ADR 0001's eight-star threshold, named in its own heading. The game grants a **league-dependent** number of bonuses and nothing in the schema knows it — verified against stored `raw_snapshots`, where neither CWL payload carries any bonus, medal or reward field. So the surface ranks and the leader supplies the count. `warLeague.id` is on the `/clans/{tag}` payload the collector already fetches and discards, which makes the count derivable later from a static league table; that is additive, not a restructure.
- **One fact is recorded: whether the bonuses have been handed out.** Not who received them — that is a record nobody asked for and the API will never confirm — but whether the job is done, which is what says whether this surface is still in use or finished. It stores as `bonuses_administered_at` on `cwl_seasons`, a column and a mutation rather than a table. It is also a **better resting marker than ADR 0002's "days since the final war's `end_time`"**, because it is an observation instead of a guess about when someone lost interest; the elapsed-time marker stays as the backstop for a season nobody ever marks.
- **One fact does not make an editing surface.** No action bar and no draft: the state is a `cm-statuschip` in the topbar and one item in the season menu, exactly how the lineup carries the day lock. The bar exists to hold "unsaved" against "not yet done in game" (#21), and neither question exists here. That is also the second use of the statuschip, which is what makes the rename off `lockchip` right.
- **A previous season is reachable from the topbar menu, not the phase strip.** ADR 0002 requires the reach; seasons accumulate without bound while the strip already carries the phase. This also surfaced a real data gap: every CWL view routes through `cwl_current_seasons`, which is the latest season and nothing else, so an earlier season is not queryable today. The menu says so rather than offering entries that would return nothing.
- **Attacks used is not on the row, and that is the row-marking rule doing real work.** An unmarked row used every attack it was given, so printing "7 of 7" beside it says the same thing twice. Stars and wars joined stay, because they are the ranking's own terms — three stars from one war and three from seven are different facts.
- **A coverage gap is scoped to the thing that varies.** A war day that never reached `warEnded` is missing for all fifteen members who were in it, so marking the row fires on most of the roster and distinguishes nobody. It belongs to the season — the eyebrow states it — and to the panel, where it explains one member's figures.
- **No new component, and no ninth icon.** The war-day record uses words rather than a tick and a cross, because #40's line is that an affordance becomes an icon and a war day's outcome is not something you press. The one change to the inventory is a promotion: the summary strip was members-only page layer and is needed here unchanged, so it moves to the system layer.
- **No action bar, from the other direction.** The members roster had nothing to save; this surface has three decisions and still nothing to save. Two surfaces, two different reasons, same conclusion — the bar belongs to the editing layer.
...
- **Amended by [#58](https://github.com/nswanger/clash-of-clans/issues/58): the mark stands alone, with no product name beside it.** #24 wrote the placement as "beside the product name", which assumed an app bar that names the product. There is no such bar — the primary nav is the page name in `cm-topbar`, so the only bar in the app names the *page*, and the mark sits at 24px in its first slot. The rule the placement was protecting is unchanged and is the one that binds: **once per screen, in the top bar, never on rows or as texture.** The product name survives in exactly one place, the auth shell, which is the one surface with no page name to carry.
