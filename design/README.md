...
- **Appearance is verified by hand against this design system, at 375px and 1280px in both themes.** There is no visual-regression tooling: for an app this size the machinery costs more than the risk. The prototypes were the exact, versioned reference for the migration off the old UI and remain the record of each lock; they are not a spec every later change is measured against, and new work fits the tokens and the inventory and may diverge from the prototype where it needs to. A surface that disagrees with the inventory is a finding to record in [`components.md`](components.md), never a licence to invent a component or a token inside one page.
