...
| `cm-routemenu` | `aria-current="page"` on the active route, `is-trailing` | **The system's disclosure menu**, in three uses — the three routes, the account control's sign-out, and a row's actions behind one `cm-iconbutton` ([#125](https://github.com/nswanger/clash-of-clans/issues/125)). Overlay, `cm-shadow-overlay`, 44px rows. `is-trailing` anchors it to the right edge, where the account control and a row's trailing control are. **A destructive action never sits inline beside a routine one on a row:** the row carries the one trigger, the menu carries the actions, the destructive one last in danger ink. The remaining inline ghosts are each a single control (Sign out, Clear all filters, Clear this list, the pager, the fresh invitation's Copy and Dismiss) or were decided on their own terms ([#102](https://github.com/nswanger/clash-of-clans/issues/102), [#114](https://github.com/nswanger/clash-of-clans/issues/114)). |
...
**A list shows at most ten rows, and everything past that is reached by narrowing** ([ADR 0024](../docs/decisions/0024-design-list-length-and-reveal.md)). `--cm-list-max-rows` is the token; `LIST_MAX_ROWS` in the React system layer is its other half, and the two must not disagree. The rule exists because a box sized from a fixed row count never resizes under a filter, never needs an inner scrollbar inside a page that already scrolls, and does not grow with the clan. Narrowing means `cm-search` on a list that is searched, and a pager on a list that is walked; that is a component decision for that surface, not an exemption. **A list showing less than everything says so** — `N of M shown`, the form the lineup's bench already uses.

**The pager exists, once, at the page layer** ([#117](https://github.com/nswanger/clash-of-clans/issues/117)). ADR 0024 anticipated "a surface whose rows are walked through rather than searched" and left it to the first surface that needed one; Admin's access log is that surface. Its shape: ten rows, `first–last of total`, two ghosts (Newer / Older), and `LIST_MAX_ROWS` is the page size so the two halves of the rule stay one number. It is `admin-pager` rather than `cm-pager` because one surface has it; a second log or history that pages is what would promote it.

**A section closed until opened is also page layer, for the same reason** ([#117](https://github.com/nswanger/clash-of-clans/issues/117)). The access log is consulted when something is wrong, not scanned daily, so it renders as a `<details>` whose `<summary>` is the `cm-section-head`, with the count kept on the head so a closed log still says how much is behind it. The disclosure glyph is `cm-routebutton-chev`, rotated as the route menu rotates it — no third chevron form. `admin-collapsible` today; a second surface wanting one promotes it.
...
| `cm-pill` | `is-success`, `is-caution`, `is-danger` | |

**`is-danger` arrived with the Admin endpoint board** ([#117](https://github.com/nswanger/clash-of-clans/issues/117)). The inventory carried success and caution only because no leader surface ever needed a danger pill — rows mark the exception with `cm-statustext`. The board's "failed" is a state word in the same slot People puts a role, and a second shape for the same job on one page is what the reuse rule exists to stop. Leader surfaces are unchanged: an unavailable member is still `cm-statustext.is-unavailable`, not a pill.
...
**Admin:** the collection-health facts list (drawn only when a run is unhealthy — [#117](https://github.com/nswanger/clash-of-clans/issues/117) applied ADR 0014's row rule to the section), the operator's endpoint board (`admin-endpoint`: six `cm-row`s always, a bounded exception to ADR 0014 recorded in the #117 spec; `cm-pill` marks, no notice, no times on rows), the run line and next-run fact (`admin-nextrun`), the invitation token block, the access log (`admin-audit`, one grid shape per row: a content-sized time column first, the event in the rest, in a fixed `4 Sep 21:10` format because a locale's 12-hour clock wrapped the column on a phone), its pager and closed section (above), the per-row error line, and the row menu (`admin-rowmenu`: one `cm-iconbutton` opening a `cm-routemenu is-trailing`, [#125](https://github.com/nswanger/clash-of-clans/issues/125) — never inline ghosts). Conformed against this inventory rather than against a prototype, because wave 3 has none for it — which makes it the inventory's third test and the one that produced the two component changes above.
